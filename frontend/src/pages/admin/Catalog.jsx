...
export default function Catalog() {
  ...
  async function handleAddCategory(e) {
    ...
    try {
      ...
      // Conveniently pre-select the category you just created for the service form below
      setForm((f) => ({ ...f, category_id: res.data.id }))
    } catch (err) {
      setCategoryError(err.response?.data?.error || 'Could not add this category.')
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 grid gap-6">
          ...
          <div className="bg-white rounded-lg border border-stone-200 p-4">
            <h2 className="font-semibold text-stone-800 mb-3 text-sm">Categories</h2>
            <div className="flex flex-wrap gap-2">
              {categories.map((c) => (
                <span key={c.id} className="text-xs px-3 py-1 rounded-full bg-stone-100 text-stone-600">
                  {c.name}
                </span>
              ))}
              {categories.length === 0 && (
                <span className="text-xs text-stone-400">No categories yet - add one on the right.</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6 h-fit">
          <form onSubmit={handleAddCategory} className="bg-white rounded-lg border border-stone-200 p-5 grid gap-3">
            <h2 className="font-semibold text-stone-800">Add a Category</h2>
            <input
              placeholder="e.g. Lighting, Transport, Cakes"
              value={newCategory}
              onChange={(e) => setNewCategory(e.target.value)}
              className="rounded-lg border border-stone-300 px-3 py-2 text-sm"
            />
            {categoryError && <p className="text-red-600 text-xs">{categoryError}</p>}
            <button
              disabled={addingCategory}
              className="rounded-full border border-burley-600 text-burley-600 px-4 py-2 text-sm hover:bg-burley-50 disabled:opacity-50"
            >
              {addingCategory ? 'Adding...' : 'Add Category'}
            </button>
          </form>

          <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-stone-200 p-5 grid gap-3">
            ...
            {categories.length === 0 && (
              <p className="text-xs text-amber-600">Add a category first (form above) before adding a service.</p>
            )}
            <input
              required
              type="number"
